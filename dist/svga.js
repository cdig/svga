(function() {
  ...
  Panel3d = class Panel3d {
    constructor(options1, scopes, fullscreenModelContainer, runningOnWebkit1) {
      this.options = options1;
      this.scopes = scopes;
      this.fullscreenModelContainer = fullscreenModelContainer;
      this.runningOnWebkit = runningOnWebkit1;
...
void main() {
    ...
    setupCanvas() {
      return new Promise((resolve, reject) => {
        ...
        if (this.options.panelSettings.fullscreen) {
          this.fullscreenModelContainer.prepend(this.container);
          this.scopes.Resize(() => {
    ...
    syncInternalTransform() {
      var currentScale, matrix, offsetX, offsetY, screenX, screenY;
      matrix = this.container.getScreenCTM();
      if (!matrix) {
        return;
      }
      ...
      offsetX = this.options.panelSettings.x * currentScale;
      offsetY = this.options.panelSettings.y * currentScale;
      this.wrapper.style.left = (screenX + offsetX) + "px";
      this.wrapper.style.top = (screenY + offsetY) + "px";
      // 3. Apply Scale and Dimensions
      this.wrapper.style.width = this.options.panelSettings.width + "px";
      this.wrapper.style.height = this.options.panelSettings.height + "px";
      
      // Use transform for the scale so the browser doesn't struggle
      this.wrapper.style.transform = `scale(${currentScale})`;
      return this.wrapper.style.transformOrigin = "top left";
    }
    ...
    createMaterial(opts) {
      var baseColorTex, mat, normalTex, ormTex;
      baseColorTex = this.textureLoader.load(this.expandResourceName(opts.baseColorTexture));
      normalTex = this.textureLoader.load(this.expandResourceName(opts.normalTexture));
      ormTex = this.textureLoader.load(this.expandResourceName(opts.oclusionRoughnessMetalicTexture));
    ...
    useMaterialForAll(mat) {
      return this.scene.traverse((child) => {
        if (child.isMesh) {
    ...
    useHDR(hdrName) {
      ...
      return loader.load(url, (texture) => {
        ...
        this.log(`HDR Loaded: ${hdrName}`);
        return this.scene.traverse((node) => {
          if (node.isMesh) {
  ...
  Model = class Model {
    ...
    restructureSVGALayers() {
      ...
      svgaClone = this.scopes.SVG.svg.cloneNode(false);
      ...
      this.scopes.SVG.svg.after(svgaClone);
      // Move the x-gui group into the other one
      svgaClone.appendChild(this.scopes.GUI.elm);
      // Shallow copy the GUI back into the original svga, this will house the 3d model if full screen
      this.fullscreenModelContainer = this.scopes.GUI.elm.cloneNode(false);
      this.scopes.SVG.root.after(this.fullscreenModelContainer);
      // Add the resize method to the svgaClone
      this.scopes.Resize(() => {
        svgaClone.setAttribute('width', window.innerWidth);
        return svgaClone.setAttribute('height', window.innerHeight);
      });
    ...
    async createPanel(options) {
      var panel, runningOnWebkit;
      await this.ensureLibrariesImported();
      runningOnWebkit = this.isWebKit();
      await this.restructureSVGALayers();
      // Now THREE and OrbitControls are guaranteed to exist
      panel = new Panel3d(options, this.scopes, this.fullscreenModelContainer, runningOnWebkit);
      await panel.setupCanvas();
      return panel;
    }

    isWebKit() {
      var isAppleVendor, isWebKitEngine;
      // Check for the vendor or the engine string
      // 'Apple Computer, Inc.' is the vendor for all iOS browsers and Safari
      isAppleVendor = (navigator.vendor != null) && navigator.vendor.indexOf('Apple') > -1;
      // Also check the User Agent for the engine name
      isWebKitEngine = /AppleWebKit/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent);
      
      // On iOS, even Chrome reports as 'Apple Computer, Inc.'
      return isAppleVendor || isWebKitEngine;
    }
...
<small settings-copyright>© CD Industrial Group Inc.</small>`);
  ...
  LiveData = class LiveData {
    ...
    useSignalingHost(host) {
      return this.webRTCTools.useSignalingHost(host);
    }
    ...
    setDescriptions(descriptionMap, aliasIndex) {
      ...
      for (n = 0, len1 = inputs.length; n < len1; n++) {
        ...
        results.push(input.addEventListener('change', (e) => {
          ...
          value = el.value;
          return this.updateChannelName(og, value); // oldName, newName
        }));
      }
      return results;
    }
...
<div class="ld-pub-container">
    ...
    _removeUI() {
      var ref, ref1;
      if ((ref = document.getElementById('live-data-root')) != null) {
        ref.remove();
      }
      return (ref1 = document.getElementById('live-data-style')) != null ? ref1.remove() : void 0;
    }
    ...
    _cacheElements() {
      this.root = document.getElementById('live-data-root');
      this.container = this.root.querySelector('.otp-container');
    ...
    _wireEvents() {
      ...
      this.inputs.forEach((input, i) => {
        input.addEventListener('input', () => {
          ...
          if (input.value && i < this.inputs.length - 1) {
    ...
    _showPlug() {
      if (this.btnPlug) {
        return this.btnPlug.style.display = 'unset';
      }
    }
    ...
    _stateDisconnectReady() {
      this._setInputsDisabled(true);
      this.btnDisconnect.style.display = 'unset';
      this.btnDisconnect.disabled = false;
      return this.btnConnect.style.display = 'none';
    }
  ...
  s.onerror = function() {
    return console.error('Failed to load Socket.IO');
  };

  s.onload = function() {
    return Make("socket.io");
  };
  ...
  WebRTCTools = class WebRTCTools {
      ...
      return this.socket.on('connect', () => {
        ...
        this.pc.oniceconnectionstatechange = () => {
          ...
          switch (this.pc.iceConnectionState) {
            ...
            case "connecting":
              return this.updateConnectionState("connecting");
            case "connected":
    ...
    cancelTaredown() {
      return clearTimeout(this.tareDownTimeout);
    }
    ...
    onConnectionStateChange(fn) {
      return this.connectionStateChange = fn;
    }

    updateConnectionState(connectionState) {
      this.connectionState = connectionState;
      if (this.connectionStateChange) {
        return this.connectionStateChange(this.connectionState);
      }
    }
